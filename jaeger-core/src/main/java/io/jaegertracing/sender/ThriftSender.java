...
import io.jaegertracing.internal.JaegerThriftSpanConverter;
...
public abstract class ThriftSender extends ThriftSenderBase implements Sender {
  ...
  public int append(Span span) throws SenderException {
    // TODO: the sender API needs a way to get access to the tracer...
    io.jaegertracing.internal.Span jaegerSpan = (io.jaegertracing.internal.Span) span;

    if (process == null) {
      process = new Process(jaegerSpan.getTracer().getServiceName());
      process.setTags(JaegerThriftSpanConverter.buildTags(jaegerSpan.getTracer().tags()));
      processBytesSize = calculateProcessSize(process);
      byteBufferSize += processBytesSize;
    }

    io.jaegertracing.thriftjava.Span thriftSpan = JaegerThriftSpanConverter.convertSpan(jaegerSpan);
